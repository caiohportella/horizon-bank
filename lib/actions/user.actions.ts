...
const { APPWRITE_USER_COLLECTION_ID: USER_COLLECTION_ID } = process.env;
const { APPWRITE_BANK_COLLECTION_ID: BANK_COLLECTION_ID } = process.env;
...
export async function createLinkToken(user: User) {
  try {
    const tokenParams = {
      ...
      language: "en",
      country_codes: ["US", "CA", "GB", "FR"] as CountryCode[],
    };
